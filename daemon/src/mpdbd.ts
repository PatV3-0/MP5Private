...
export class Daemon{
    ...
    public getUserMail(username: string){
        const usersFilePath = path.join('databases', 'admin', 'users.json');
    ...
    try {
      ...
      switch (request.action) {
        case "login":
            ...
            try {
              ...
              if (loginSuccess) {
                ...
                console.log("Generating API key for logged in user");
                let uname = request.payload.username;
                let umail = this.getUserMail(uname);
                let uid = this.getUserID(uname);

                let apiData = {_id:uid, username:uname, email:umail};
                let apiReg = new AuthenticationHandler();
                let uapiKey = apiReg.generateAPIkey(apiData);

                response = {
                    status: 200,
                    message: "User login details correct",
                    data: { 
                        hashedPass: request.payload.password,
                        apiKey: uapiKey } 
                };
              } else {
        ...
        case "getAPIkey":
            ...
            break;
        case "revokeAPIkey":
            const {username , password, api_key} = request.payload; 
            let revokeHandler = new AuthorizationHandler();

            // verify login before anything else
            
            const loginCheck = await revokeHandler.checkLogin({username, password});
            if (!loginCheck){
                response = { status:400 , message : "Invalid username or password.", data:null};
                break; // exit
            }
            // revoke ApiKey
            
            const revokeSuccess = revokeHandler.revokeAPIkey({ apiKey: api_key });
            if (revokeSuccess) {
                response = {
                    status: 200, message: "API key successfully revoked", data: null,
                };
            } else {
                response = {
                    status: 400, message: "Invalid API key or revocation failed", data: null,
                };
            }
            break;
        case "getUser":
            try {
                ...
                const requestingUserID = this.getUserID(jwtToken["username"]);
                const userID = requestingUserID;
        
                // Check if requester is admin or accessing their own account
                if (!this.authorizationHandler.isAdmin({ apiKey }) && requestingUserID !== userID) {
                    response = {
                        status: 403,
        ...
        case "updateUser":
            try {
                ...
                const { apiKey, userData } = request.payload;
                const requestingUserID = this.getUserID(jwtToken["username"]);
                const userID = requestingUserID;
                ...
                const usersFilePath = path.join('databases', 'admin', 'users.json');
                const fileManager = new FileManager();
                let users = fileManager.readFile(usersFilePath);
                ...
                //don't update
                const sensitiveFields = ['_id', 'username', 'password', 'admin'];
                ...
                fileManager.updateFile(usersFilePath, users);
        
                // Generate new API key with updated info
                const authHandler = new AuthenticationHandler();
                const newApiKey = authHandler.generateAPIkey({
                    _id: userID,
                ...
                response = {
                    status: 200,
                    message: "User updated successfully. New API key generated.",
